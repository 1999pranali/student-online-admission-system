...
$id=$_GET['sid'];
if(isset($id)){
  $q="select * from subject where s_id=$id;";
...
?>
  <div class="container">
         ...
         <div class="form-group">
            <div class="form-label-group">
             ...
             <label>Subject Name:</label>
              <input type="text" name="sname" class="form-control" placeholder="Subject Name" required="required" autofocus="autofocus" value="<?php echo $r[1];?>">
              </div>
              </div>
              <input type="hidden" name="sid" value="<?php echo $id;?>">
               
             <button class="btn btn-primary btn-sm" type="submit" name="btnsub">Save Subject</button>
              </div>
          </div>
          </div>
        </form></br></br>
        ...
        <tbody>
          <?php
            include("connection.php");
            $q="select subject.s_id ,s_name ,c_name from course ,subject where course.c_id=subject.c_id;";
            $result=mysqli_query($con,$q);
      ...
      <td><a href="subject.php?sid=<?php echo $r[0];?>"><button type="button" class="btn btn-primary btn-sm" name="btnedit">Edit</button></a></td>
      <td><form action="deletesubject.php" method="POST"><button type="submit" class="btn btn-primary btn-sm" name="btndelete" value="<?php echo $r[0];?>">Delete</button></form></td>
      </tr>
      <?php
        }
        ?>
...
else
{
  ?>
        ...
        <div class="card-header">Subject</div>
        <div class="card-body">
        <form method="POST" action="subject1.php"  >
         <div class="form-group">
            <div class="form-label-group">
            ...
            <div class="form-label-group">
             <label>Subject Name:</label>
              <input type="text" name="sname" class="form-control" placeholder="Subject Name" required="required" autofocus="autofocus">
              </div>
              </div>
             <button class="btn btn-primary btn-sm" type="submit" name="btnsub">Save Subject</button>
              </div>
